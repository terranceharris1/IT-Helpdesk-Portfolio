# IT Help Desk Portfolio – Terrance Harris

## Overview
This portfolio demonstrates hands-on experience with IT support tools and concepts including ServiceNow, Active Directory, and troubleshooting real-world IT issues.

---

## Skills Demonstrated
- Incident management (ServiceNow)
- Ticket lifecycle handling (creation → resolution)
- User account troubleshooting
- Documentation and work notes
- Basic networking troubleshooting

---

## Projects

### 🔐 Password Reset & Account Unlock (ServiceNow)

**Issue:**  
User was unable to log into their workstation due to account lockout.

**Tools Used:**  
- ServiceNow  
- Active Directory (lab environment)

**Steps Taken:**
1. Created incident ticket in ServiceNow  
2. Investigated login issue  
3. Identified account lockout  
4. Reset password and unlocked account  


**Resolution:**  
User successfully logged in after password reset and account unlock.

---
![Ticket Creation](ServiceNow-Ticket-Creation.png)
![Incident Notes](Incident-Work-Notes.png)
![Account Unlock](Account-Unlock.png)
![Ticket Screenshot](Password-Reset.PNG)
![Successful Login](Successful-Login.png)
![Resolution Screenshot](Ticket-Resolution.png)

## More projects coming soon...
